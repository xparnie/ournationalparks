...
const Carousel = ({ items }) => {
    ...
    const onSlideClickHandler = (direction) => {
        ...
        if (direction === 'left') {
            index = (currentImageIndex <= 0) ? zeroTotalCarouselItems : currentImageIndex - 1;
        } else {
            index = (currentImageIndex === zeroTotalCarouselItems) ? 0 : currentImageIndex + 1;
        }
    ...
    return (
        <div className="carousel">
            ...
            <Arrow className='arrow next' direction='right' clickFunction={onSlideClickHandler} glyph='&#xf054;' />
            <Arrow className='arrow previous' direction='left' clickFunction={() => onSlideClickHandler('left')} glyph='&#xf053;' />
        </div>
    );
};
